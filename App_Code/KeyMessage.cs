//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version:4.0.30319.18051
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


    using System;
    using System.Text;
    using System.Data;
    using System.Data.SqlClient;
    
    
    public class KeyMessage {
        
        private object _KeyID;
        
        private object _KeyTitle;
        
        private object _KeyPage;
        
        private object _KeyFileName;
        
        private object _CreatedBy;
        
        private object _CreatedDate;
        
        private object _ModifiedBy;
        
        private object _ModifiedDate;
        
        private object _PresentationID;
        
        public KeyMessage() {
        }
        
        public virtual object KeyID {
            get {
                return this._KeyID;
            }
            set {
                this._KeyID = value;
            }
        }
        
        public virtual object KeyTitle {
            get {
                return this._KeyTitle;
            }
            set {
                this._KeyTitle = value;
            }
        }
        
        public virtual object KeyPage {
            get {
                return this._KeyPage;
            }
            set {
                this._KeyPage = value;
            }
        }
        
        public virtual object KeyFileName {
            get {
                return this._KeyFileName;
            }
            set {
                this._KeyFileName = value;
            }
        }
        
        public virtual object CreatedBy {
            get {
                return this._CreatedBy;
            }
            set {
                this._CreatedBy = value;
            }
        }
        
        public virtual object CreatedDate {
            get {
                return this._CreatedDate;
            }
            set {
                this._CreatedDate = value;
            }
        }
        
        public virtual object ModifiedBy {
            get {
                return this._ModifiedBy;
            }
            set {
                this._ModifiedBy = value;
            }
        }
        
        public virtual object ModifiedDate {
            get {
                return this._ModifiedDate;
            }
            set {
                this._ModifiedDate = value;
            }
        }
        
        public virtual object PresentationID {
            get {
                return this._PresentationID;
            }
            set {
                this._PresentationID = value;
            }
        }
    }
    
    public class KeyMessageCollection : System.Collections.CollectionBase {
        
        public virtual KeyMessage this[int index] {
            get {
                return ((KeyMessage)(List[index]));
            }
            set {
                List[index] = value;
            }
        }
        
        public virtual void Add(KeyMessage Item) {
            List.Add(Item);
        }
        
        public virtual void Remove(KeyMessage Item) {
            List.Remove(Item);
        }
    }

